...
void curve25519_expand(bignum25519 out, const unsigned char in_arg[32]) {
	// align the input array to 4 byte make the funciton working in every
	// kinds of call stack
	uint8_t in[32] __attribute__ ((aligned (4))) = {0};
	memcpy(in, in_arg, sizeof(in));
