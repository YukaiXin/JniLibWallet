...
#include "bleProtocol/fastmode.c"
#include <stdio.h>
#include <stdlib.h>
...
bool receiveBuff(int pos,unsigned char *data, int len, char* mac){
    ...
    jmethodID jmethodID1 = _env->GetMethodID(fastClass, "RecieveBuff", "([BILjava/lang/String;)V");
    jstring bleMac = _env->NewStringUTF(mac);

    _env->CallVoidMethod(gobal_obj, jmethodID1, dataArray, len, bleMac);

    _env ->ReleaseByteArrayElements(dataArray, jbyteP,  JNI_ABORT);
    _env -> ReleaseStringUTFChars(bleMac, mac);
    return false;
}
...
(JNIEnv *env, jobject jobj, jbyteArray data, jint len, jstring mac){


    gobal_obj = env->NewGlobalRef(jobj);
    char* deviceMac = const_cast<char *>(env->GetStringUTFChars(mac, JNI_FALSE));
    unsigned char* buffer = reinterpret_cast<unsigned char *>(env->GetByteArrayElements(data, 0));
    return onMRecieve(buffer, len, sendBuff, receiveBuff, deviceMac);
}
